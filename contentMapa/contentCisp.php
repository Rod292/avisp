        ...
        <div>
            <div class="container-fluid">
                ...
                <div class="col-md-12">
                    <form name="formDados" id="formDadosCisp" onsubmit="return validateForm()">
                        <div class="col-md-12">
        ...
        function SetupMap(){
                ...
                var json = <?php require_once('dados/insereDadosCisp.php'); ?>;
                var geojsonData = <?php require_once('geojson/limite_cisp2.geojson'); ?>;

                var lookup = {};
                var totalCrimes = [];
                json.forEach(function(item) {
                if(item.iddelegacia) {
                    lookup[item.iddelegacia] = item;
                    totalCrimes[item.iddelegacia] = Number(lookup[item.iddelegacia].qtd);       
                    }
                })
                
                geojsonData.features.forEach(function(aux) {
                if (lookup[aux.properties.DP]) {
                    aux.properties = lookup[aux.properties.DP];
                }
                })
                ...
                var choroplethLayer = L.choropleth(geojsonData, {
                    valueProperty: 'qtd', // which property in the features to use
                ...
                var geoJsonLayer = new L.geoJSON(geojsonData, {
                    style: style2,
                            ...
                            layer.bindTooltip(
                                ...
                                '<b>CISP: </b>' + props.iddelegacia + '<br />' +
                                '<b>Quantidade: </b>' + props.qtd + '<br />' +
